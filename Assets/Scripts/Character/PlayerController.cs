...
namespace YEONJI.Kindergarten
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Start()
        {
            currentCell = InGameCore.GRID.WorldToGrid(transform.position);
...
#if UNITY_EDITOR
            if (Input.GetMouseButtonDown(0) && InGameCore.ROUND.roundStart)
            {
        ...
        private IEnumerator FollowPath(List<Vector2Int> path)
        {
            ...
            if (path.Count > 1)
            {
                Vector3 firstTarget = InGameCore.GRID.GridToWorld(path[1].x, path[1].y);
            ...
            {
                Vector3 target = InGameCore.GRID.GridToWorld(path[i].x, path[i].y);
        ...
        private bool FindAdjacentCell(Vector2Int kidCell, out Vector2Int result)
        {
            ...
            foreach (var dir in dirs)
            {
                ...
                var cell = InGameCore.GRID.GetCell(adj.x, adj.y);
                if (cell == null) continue;
                if (cell.zone != ZoneType.MainRoom) continue;
                if (!cell.IsWalkable) continue;
                if (InGameCore.KIDS.GetKidAgent(adj) != null) continue;
