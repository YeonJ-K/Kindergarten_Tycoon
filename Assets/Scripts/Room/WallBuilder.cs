...
namespace YEONJI.Kindergarten
{
    ...
    public class WallBuilder : MonoBehaviour
    {
        ...
        {
            var cell = InGameCore.GRID.GetCell(x, y);
        ...
        void BuildBorder()
        {
            for (int x = 0; x < InGameCore.GRID.mapWidth; x++)
            for (int y = 0; y < InGameCore.GRID.mapHeight; y++)
            {
        ...
        void BuildVertical()
        {
            int midX = InGameCore.GRID.mapWidth / 2;

            // 왼쪽 절반
            for (int x = 0; x < midX; x++)
            {
                bool active = false;
                for (int y = InGameCore.GRID.mapHeight - 1; y >= 0; y--)
                {
            ...
            // 오른쪽 절반
            for (int x = midX; x < InGameCore.GRID.mapWidth; x++)
            {
                bool active = false;
                for (int y = InGameCore.GRID.mapHeight - 1; y >= 0; y--)
                {
        ...
        void BuildHorizontal()
        {
            ...
            for (int y = 0; y < InGameCore.GRID.mapHeight - 1; y++)
            {
        ...
        void Draw()
        {
            ...
            foreach (var kv in placed)
            {
                ...
                go.transform.position = InGameCore.GRID.GridToWorld(kv.Key.x, kv.Key.y);
        ...
        void OnDrawGizmos()
        {
            if (!showDebugLabels || placed == null || placed.Count == 0 || InGameCore.GRID == null) return;
    #if UNITY_EDITOR
            ...
            foreach (var kv in placed)
            {
                Vector3 pos = InGameCore.GRID.GridToWorld(kv.Key.x, kv.Key.y);
