...
namespace YEONJI.Kindergarten
{
    public class RoundManager : BaseManager
    {
        ...
        private List<int> GetKidsNum(int levelPerKidsNum)
        {
            ...
            for (int i = 0; i < InGameCore.KIDS.KidsPrefabs.Count; i++)
            {
                all.Add(i);
            }
        ...
        private IEnumerator Entering()
        {
            ...
            {
                GameObject go = Instantiate(InGameCore.KIDS.KidsPrefabs[kidsNum[i]],
                    InGameCore.GRID.GridToWorld(kidsSpawnPos.x, kidsSpawnPos.y), Quaternion.identity);
                KidAgent agent = go.GetComponent<KidAgent>();
                agent.Init(kidsSpawnPos);
                InGameCore.AI.Register(agent);
